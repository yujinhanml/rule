...
#             # prompt = f"Change the digit at the {region_names[changed_index]} position to {new_digit}."
#             prompt_new = f"An image of digits {pos_digits}."
...
class DigitSeqNegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            image_neg = self.transform(image_neg)
        # return image_pos, entry["prompt_new"], entry["prompt_old"], entry["image_path_neg"], image_neg
        return image_pos, entry["prompt"], entry["image_path_neg"], image_neg, entry["Mask_index"]
...
class MLPProjector(nn.Module):
    def __init__(self, input_tokens=1, input_dim=input_dim, output_tokens=256, output_dim=32, hidden_dim=2048, depth=2):
        super().__init__()
...
wandb_logger = wandb.init(
    project="tokenizer",
    name=f"Edit_res_condition_{vae_type}-connector{connector_type}-mnist-mnist-epoch{num_epochs}-recon_weight{recon_weight}-ori_weight{ori_weight}-perceptual{perceptual_loss_weight}-resweight{res_weight}-semanticweight{semantic_weight}-lastoken-prompt"
)
...
for epoch in trange(num_epochs):
    ...
    for img_tensor, text_prompt, img_path, img_neg, mask_index in tqdm(train_loader, desc=f"Epoch {epoch}"):
        B = len(text_prompt)
        ...
        H = W = int(N ** 0.5)
        proj = project_latents.view(B, H, W, C)
        base = quant_neg.view(B, H, W, C)
        mask = torch.zeros((B, H, W, 1), device=project_latents.device)
        for b in range(B):
            idx = mask_index[b]
            if idx == 0:  # top-left
                mask[b, :H//2, :W//2, :] = 1
                ...
                raise ValueError(f"Invalid mask index: {idx}")
        project_latents = (proj * mask).view(B, N, C)
        quant_neg = (base * (1 - mask)).view(B, N, C)
        estimation_input = project_latents+ (quant_neg).detach()
        # === Step 4: compute loss ===
        # semantic_loss = F.mse_loss(project_latents, ((quant - quant_neg)*res_weight).detach())
        semantic_loss = F.mse_loss(estimation_input , quant.detach())


        # triplet_loss_fn = torch.nn.TripletMarginLoss(margin=1.0)
        # project_latents_con = project_latents + quant_neg
        # anchor = project_latents_con.view(B, -1)
        # positive = quant.view(B, -1)
        # negative = quant_neg.view(B, -1)
        # semantic_loss = triplet_loss_fn(anchor, positive, negative)

        # print("project_latents mean:", project_latents.mean(), "quant - quant_neg mean:", ((quant - quant_neg)/res_weight).mean())

        
        # B, N, C = quant.shape
        # H = W = int(N ** 0.5)
        quant_input = quant.permute(0, 2, 1).view(B, C, H, W)  # [B, 32, 16, 16]
        estimation_input  = estimation_input .permute(0, 2, 1).view(B, C, H, W)
        # project_latents_input = project_latents.permute(0, 2, 1).view(B, C, H, W)

        # F.mse_loss(project_latents_con, quant.detach())
        # print("quant_input.shape:",quant_input.shape,"project_latents_input.shape:",project_latents_input.shape)
        samples_real = samples_con(quant_input, xT, vae.decoder)
        samples_fake = samples_con(estimation_input, xT, vae.decoder)
        # samples_fail = samples_con(quant_neg_input, xT, vae.decoder)

        recon_pixel_loss = F.mse_loss(samples_fake, samples_real)
        ori_pixel_loss = F.mse_loss(samples_fake, img_tensor)
        # anchor = samples_fake.view(B, -1)
        # positive = samples_real.view(B, -1)
        # negative = samples_fail.view(B, -1)
        # recon_pixel_loss = triplet_loss_fn(anchor, positive, negative)

        # anchor = samples_fake.view(B, -1)
        # positive = img_tensor.view(B, -1)
        # negative = samples_fail.view(B, -1)
        # ori_pixel_loss = triplet_loss_fn(anchor, positive, negative)
        ...
        total_loss = semantic_weight * semantic_loss + recon_weight * recon_pixel_loss + ori_weight * ori_pixel_loss + perceptual_loss_weight * ori_perceptual_loss
    ...
    if epoch % epoch_interval == 0:
        ...
        ckpt_dir = f"/cpfs04/user/hanyujin/rule-gen/rule_tokenizer/connector/checkpoints/Edit_res_condition_{vae_type}-connector{connector_type}_weight{recon_weight}-ori_weight{ori_weight}-perceptual{perceptual_loss_weight}_resweight{res_weight}_sematicweight{semantic_weight}-lastoken-prompt-nopromptdiff"
