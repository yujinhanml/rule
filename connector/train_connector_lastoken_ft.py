    ...
    def __init__(self, pos_dir, neg_dir, transform=None):
        ...
        for fname in os.listdir(pos_dir):
            ...
            if prompt_type == 'full':
                ...
                prompt = f"An image of digits [{', '.join(map(str, pos_digits))}]."
...
class DigitSeqDataset(Dataset):
    def __init__(self, image_dir, transform=None, shuffle_ratio=1.0):
        ...
        self.total = len(self.samples)
        self.all_label_strs = list(self.label_to_paths.keys())
    ...
    def __getitem__(self, idx):
        digits, label_str, image_path_prompt = self.samples[idx]
        # prompt = f"An image of digits {label_str}"
        prompt = f"An image of digits [{', '.join(map(str, digits))}]."
        ...
        # === img_path 是一个不同类别的图像路径 ===
        if random.random() < self.shuffle_ratio:
            # 选取与当前 label_str 不同的标签
            different_label_strs = [lbl for lbl in self.all_label_strs if lbl != label_str]
            if not different_label_strs:
                raise ValueError(f"No different labels found for {label_str}")
            random_label_str = random.choice(different_label_strs)
            shifted_candidates = self.label_to_paths[random_label_str]
            img_path = random.choice(shifted_candidates)
        else:
            img_path = image_path_prompt  # 不打乱，保留原图路径
        image_neg = Image.open(img_path).convert("RGB")
        if self.transform:
            # image_pos = self.transform(image_pos)
            image_neg = self.transform(image_neg)

        return image, prompt, img_path,image_neg
...
data_path_neg = "/cpfs04/user/hanyujin/rule-gen/datasets/mnist-mnist-seq-neg-1/images"
data_path_rand = "/cpfs04/user/hanyujin/rule-gen/datasets/mnist-mnist-seq/images"

dataset_neg = DigitSeqNegDataset(pos_dir=data_path_pos, neg_dir=data_path_neg, transform=transform)
...
for epoch in trange(num_epochs):
    for img_tensor, text_prompt, img_path, img_neg in tqdm(train_loader, desc=f"Epoch {epoch}"):
        ...
        #     videos=video_inputs,
        #     return_tensors="pt",
        #     padding="longest",     # 动态 pad 到最长
        #     truncation=True,       # 防止超过模型最大长度
        #     max_length=None        # 或指定最大限制，如 512
        # ).to(device)
        inputs = processor(
            text=texts,
            images=image_inputs,
