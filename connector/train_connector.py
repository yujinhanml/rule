...
class MLPProjector(nn.Module):
    def __init__(self, input_tokens=max_token, input_dim=input_dim, output_tokens=256, output_dim=32, hidden_dim=2048, depth=2):
        super().__init__()
...
wandb_logger = wandb.init(
    project="tokenizer",
    name=f"{vae_type}-connector{connector_type}-mnist-mnist-epoch{num_epochs}-recon_weight{recon_weight}-ori_weight{ori_weight}-perceptual{perceptual_loss_weight}-shuffle{shuffle_ratio}"
)
...
for epoch in trange(num_epochs):
    for img_tensor, text_prompt, img_path in tqdm(train_loader, desc=f"Epoch {epoch}"):
        B = len(text_prompt)
    ...
    # === 拼图可视化 ===
    row_pairs = [
    torch.cat([t, r, f], dim=2)
    for t, r, f in zip(truth_buffer[:8], real_buffer[:8], fake_buffer[:8])
    ]
    ...
    if epoch % epoch_interval == 0:
        ...
        ckpt_dir = f"/cpfs04/user/hanyujin/rule-gen/rule_tokenizer/connector/checkpoints/{vae_type}-connector{connector_type}_weight{recon_weight}-ori_weight{ori_weight}-perceptual{perceptual_loss_weight}_shuffle{shuffle_ratio}"
