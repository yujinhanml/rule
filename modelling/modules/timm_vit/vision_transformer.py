...
from modelling.modules.timm_vit.rope_utils import apply_rotary_emb
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.flash_attn = True if flash_attn_qkvpacked_func is not None else False
    ...
    def forward(self, x: torch.Tensor, 
                attn_mask=None, freqs_cis=None, num_prefix_tokens=1, num_latent_tokens=32) -> torch.Tensor:
        B, N, C = x.shape
        ...
        if self.fused_attn:
            ...
            if freqs_cis is not None:
                if num_latent_tokens == 0:
                    q[:, :, num_prefix_tokens:, :], k[:, :, num_prefix_tokens:] = apply_rotary_emb(q[:, :, num_prefix_tokens:], k[:, :, num_prefix_tokens:], freqs_cis=freqs_cis)
                else:
                    q[:, :, num_prefix_tokens:-num_latent_tokens, :], k[:, :, num_prefix_tokens:-num_latent_tokens] = apply_rotary_emb(q[:, :, num_prefix_tokens:-num_latent_tokens], k[:, :, num_prefix_tokens:-num_latent_tokens], freqs_cis=freqs_cis)
            x = F.scaled_dot_product_attention(
                q, k, v,
                dropout_p=self.attn_drop.p if self.training else 0.,
            )
            x = x.transpose(1, 2).reshape(B, N, C)
        elif self.flash_attn and qkv.dtype != torch.float32:
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: torch.Tensor = None, freqs_cis=None, num_prefix_tokens=1, num_latent_tokens=32) -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), attn_mask, freqs_cis, num_prefix_tokens, num_latent_tokens)))
...
class VisionTransformer(nn.Module):
    ...
    def _pos_embed(self, x: torch.Tensor, use_ape=True) -> torch.Tensor:
...
@register_model
def vit_small_patch14_dinov2_movqv2(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-S/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=384, depth=12, num_heads=6, init_values=1e-5, block_fn=MoVQBlockv2)
    model = _create_vision_transformer(
        'vit_small_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_base_patch14_dinov2_movqv2(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-B/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, block_fn=MoVQBlockv2)
    model = _create_vision_transformer(
        'vit_base_patch14_dinov2', pretrained=pretrained,  **dict(model_args, **kwargs))
    return model
