...
#     num_classes=1000,
# ):
#     # 统一使用 float32 计算（与模型精度一致）
#     _dtype = torch.float32  # 强制使用 float32
#     device = latents.device

#     # 初始化 x_next 为 float32
#     x_next = latents.to(_dtype)
    
#     # 时间步长转为 float32
#     t_steps = torch.linspace(1., 0.04, num_steps, dtype=_dtype, device=device)
#     t_steps = torch.cat([t_steps, torch.tensor([0.], dtype=_dtype, device=device)])
...
#                 model_input = torch.cat([x_cur] * 2, dim=0).contiguous()  # 强制连续
#                 y_cur = torch.cat([y, y_null], dim=0).contiguous()
#             else:
#                 model_input = x_cur
#                 y_cur = y

#             time_input = t_cur * torch.ones(model_input.size(0), device=device, dtype=_dtype)
            
#             # 计算前向传播（保持 float32）
#             v_cur = model(model_input, time_input, y=y_cur)[0]
            
#             # 确保后续张量连续
#             s_cur = get_score_from_velocity(v_cur, model_input, time_input, path_type).contiguous()
#             diffusion = compute_diffusion(t_cur).to(_dtype)
            
#             # 更新 x_next
#             d_cur = v_cur - 0.5 * diffusion * s_cur
#             if cfg_scale > 1.0 and guidance_low <= t_cur <= guidance_high:
#                 d_cur_cond, d_cur_uncond = d_cur.chunk(2)
#                 d_cur = (d_cur_uncond + cfg_scale * (d_cur_cond - d_cur_uncond)).contiguous()

#             x_next = x_cur + d_cur * dt + torch.sqrt(diffusion) * torch.randn_like(x_cur) * torch.abs(dt).sqrt()
