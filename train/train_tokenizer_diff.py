...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/{args.dataset}-exp{experiment_index:03d}-{model_string_name}"  # Create an experiment folder
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):  
                loss_gen, codebook_loss, info = vq_model(imgs)
            ...
            if train_steps % args.log_every == 0:
                ...
                if rank == 0 and wandb_logger is not None:
                    log_dict = {"lr": optimizer.param_groups[0]["lr"], "train_loss": avg_loss, 'diff_loss': loss_gen.item()}
