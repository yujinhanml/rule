...
class MaskCollator(object):
    ...
    def _gen_shadow_mask(self, img_tensor, generator, aspect_ratio_scale):
        ...
        y_max = (y + h) // self.patch_size + 1


        new_w = self.width - x_min  

        # 从 scale, aspect_ratio_scale 中采样
        _rand = torch.rand(1).item() 
        min_ar, max_ar = aspect_ratio_scale
        aspect_ratio = min_ar + _rand * (max_ar - min_ar)
        
        new_h = 7 #int(round(new_w * aspect_ratio))
        ...
        return (y_max, x_min, new_h, new_w)
    ...
    def _sample_block_mask(self, b_size, shadow_param= None, acceptable_regions=None):
        ...
        while not valid_mask:
            ...
            if shadow_param is not None:
                bottom,left,h,w = shadow_param
                mask[bottom-h:bottom, left:left+w] = 1
        ...
        if shadow_param is not None:
            mask_complement[bottom-h:bottom, left:left+w] = 0
        else:
    ...
    def __call__(self, batch):
        ...
        for img_idx in range(B):
            ...
            shadow_mask = self._gen_shadow_mask(img_tensor,generator=g,
            aspect_ratio_scale=self.aspect_ratio)
            ...
            for _ in range(self.nenc):
                mask, _ = self._sample_block_mask(e_size, acceptable_regions=acceptable_regions,shadow_param=None)
                masks_e.append(mask)
